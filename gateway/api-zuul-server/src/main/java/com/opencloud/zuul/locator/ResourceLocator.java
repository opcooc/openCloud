...
import com.opencloud.zuul.service.feign.SystemAuthorityServiceClient;
...
public class ResourceLocator implements ApplicationListener<RemoteRefreshRouteEvent> {
    ...
    public ResourceLocator(JdbcRouteLocator zuulRoutesLocator, RateLimitProperties rateLimitProperties, SystemAuthorityServiceClient systemAuthorityServiceClient, GatewayServiceClient gatewayServiceClient) {
    ...
    public void loadAuthority() {
        ...
        try {
            // 查询所有接口
            List<AuthorityResource> list = systemAuthorityServiceClient.findAuthorityResource().getData();
            if (list != null) {
    ...
    protected Map<String, List<RateLimitProperties.Policy>> loadRateLimitPolicy() {
        ...
        try {
            List<RateLimitApi> list = gatewayServiceClient.getRateLimitApiList().getData();
            if (list != null) {
